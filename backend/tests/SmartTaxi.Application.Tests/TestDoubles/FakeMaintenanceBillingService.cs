...
/// <summary>Mirrors FakeAdvertisingBillingService exactly — same settlement-recovery test surface.</summary>
public sealed class FakeMaintenanceBillingService : IMaintenanceBillingService
{
    private readonly HashSet<Guid> _settledRequestIds = [];

    public List<(Guid RequestId, Guid OwnerUserId, Guid GarageUserId, decimal Amount)> SettledCalls { get; } = [];
        ...
        CancellationToken cancellationToken)
    {
        ...
        SettledCalls.Add((requestId, ownerUserId, garageUserId, amount));
